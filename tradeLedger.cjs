...
function normalizeAgentMemoryKindForRetention(entry) {
  const raw = String(entry?.kind || '').trim().toLowerCase();
  return raw || 'unknown';
}
...
class TradeLedger {
  ...
  _load() {
    try {
      ...
      this.state = {
        ...
        entries,
        memories,
        agentMemories: trimAgentMemoriesWithPolicy(agentMemories),
        agentMemoriesArchive,
        experimentNotes,
        optimizerWinners,
        researchSessions,
  ...
  async archiveAgentMemories({ cutoffMs, kinds, keepRecentPerKind = 0, keepLocked = true } = {}) {
    return this._withLock(async () => {
      ...
      if (active.length === 0) {
        return { ok: true, moved: 0, rows: archive.length, path: this.filePath };
      }
