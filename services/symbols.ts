const normalizeSymbolRaw = (value: any) => {
...
const stripSymbolSuffix = (value: string) => {
  if (!value) return '';
  return value.split('.')[0] || value;
};

export const normalizeSymbolKey = (value: any) => {
  const raw = normalizeSymbolRaw(value);
  if (!raw) return '';
  return stripSymbolSuffix(raw);
};

export const normalizeSymbolLoose = (value: any) => {
  const base = normalizeSymbolKey(value);
  if (!base) return '';
  return base.replace(/[^A-Z0-9]/g, '');
};

export const buildSymbolKeyVariants = (value: any) => {
  const raw = normalizeSymbolRaw(value);
...
export const normalizeTimeframeKey = (value: any) => {
