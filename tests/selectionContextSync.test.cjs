...
test('signal panel exposes explicit focus callback wiring', () => {
  ...
  assert.equal(signal.includes('onDiscussInChat?: (signal: SignalEntry) => void;'), true);
  ...
  assert.equal(signal.includes('Discuss in Chat'), true);
});
...
test('app publishes immediate selection context for signal and academy', () => {
  ...
  assert.equal(app.includes('const [activeSignalThreadId, setActiveSignalThreadId] = useState<string | null>(null);'), true);
  assert.equal(app.includes("openSidebarMode('chartchat');"), true);
  assert.equal(app.includes("setActiveSignalThreadId(signalId);"), true);
...
test('panel props route through selection context handlers', () => {
  ...
  assert.equal(app.includes("onDiscussInChat={(signal) => openSignalThreadInChat(signal, { originPanel: 'signal' })}"), true);
