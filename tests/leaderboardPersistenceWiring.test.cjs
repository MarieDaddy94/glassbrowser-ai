...
test('rank panel reads persisted scorecards/signal outcomes and avoids hidden polling loops', () => {
  ...
  assert.equal(app.includes("const refreshSignalHistory = useCallback(async (opts?: { force?: boolean; limit?: number }) => {"), true);
  assert.equal(app.includes("ledger.listAgentMemory({ limit, kind: 'signal_history' })"), true);
  assert.equal(app.includes("setSignalHistory(entries);"), true);
